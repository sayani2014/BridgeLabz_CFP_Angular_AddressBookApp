...
export class AddComponent implements OnInit {
   ...
   *          disable() is used to disable the input field so that the user is not able to edit the fields.
   */


  editform(): void {
    if (this.route.snapshot.params['id'] != undefined) {
      this.addressFormGroup.controls['name'].disable();
      this.addressFormGroup.controls['phoneNo'].disable();
      this.id = this.route.snapshot.params['id'];
      this.httpService.getDataByID(this.id).subscribe(x => {
        this.addressFormGroup.patchValue(x.data);
        if(x.data.state) {
          this.cityList(x.data.state);
        }
      });
    }  
  }
  ...
  /**
   * Purpose: cityList() populates the city based on the stateName selected by the user.
   * 
   ...
   *          cityList() populates the city based on the stateName selected by the user.
   * 
   * @param state name of the state which is selected by the user from the dropdown.
   */
   ...
   *          enable() is used to enable the disable input fields of the Edit form.
   *          The invalid property checks whether the form fields value are valid or not.
  ...
  onSubmit() {
    this.addressFormGroup.controls['name'].enable();
    this.addressFormGroup.controls['phoneNo'].enable();
    this.addressbook = this.addressFormGroup.value;
    if(this.addressFormGroup.invalid) {
      this._snackBar.openSnackBar("Please complete the person details!",'X','red-snackbar');
    } else {
      if(this.route.snapshot.params['id'] != undefined) {
